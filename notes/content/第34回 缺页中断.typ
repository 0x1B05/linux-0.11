...
有个问题，我们仅仅将 `/bin/sh` 文件的头部加载到了内存，其他部分并没有进行加载，那我们怎么执行到的 `/bin/sh` 的程序指令？
#figure(caption: [])[#image("images/chapter34-1.png", width: 50%)]

== 跳转到一个不存在的地址会发生什么

`/bin/sh` 这个文件并不是 Linux 0.11 源码里的内容，Linux 0.11 只管按照 `a.out` 这种格式去解读它，跳转到 `a.out` 格式头部数据结构 `exec.a_entry` 所指向的内存地址去执行指令。所以这个 `a_entry` 的值是多少，完全取决于硬盘中 `/bin/sh` 这个文件是怎么构造的了，假设它为 0，这表示随后的 CPU 将跳转到 0 地址处进行执行。(这个 0 仅仅表示逻辑地址，既没有进行分段，也没有进行分页。)由于我们现在所处的代码是属于进程 2，所以逻辑地址 0 通过分段机制映射到线性地址空间，就是 `0x8000000`，表示 128M 位置处。
#figure(caption: [])[#image("images/chapter34-2.png", width: 30%)]

128M 这个线性地址，随后将会通过分页机制的映射转化为物理地址，定位到最终的真实物理内存。可128M 这个线性地址并没有页表映射它，因为我们除了 `/bin/sh` 文件的头部加载到了内存外，其他部分并没有进行加载操作。
...
首先，缺页产生的线性地址，之前假设过了，是 `0x8000000`，也就是进程 2 自己线性地址空间的起始处 128M 这个位置。由于我们的页表映射是以页为单位的，所以首先计算出 `address` 所在的页，其实就是完成一次 4KB 的对齐。
...
所以偏移地址 `tmp` 计算后等于 `0`，这和我们之前假设的 `a_entry = 0` 是一致的。接下来就是寻找一个空闲页。
...
从硬盘首先从 `0` 内存地址开始读，应该就对应着这个文件 `0` 号数据块，当然由于 `/bin/sh` 这个 `a.out` 格式的文件使用了 `1` 个数据块作为头部 `exec` 结构，所以我们跳过头部，从文件 `1` 号数据块开始读。
...
因为仅仅是申请了物理内存页，并且把硬盘数据复制了进来，但我们并没有把这个物理内存页和线性地址空间的内存页进行映射，也就是没建立相关的页表。
